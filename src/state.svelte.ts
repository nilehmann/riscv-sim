...
export type LoadError =
    | { kind: "ParseError"; raw: string; label: string }
...
export class SimulationState {
    ...
    loadProgram(prog: Program): void {
        ...
        if (assembled instanceof ParseError) {
            this.loadError = { kind: "ParseError", raw: assembled.raw, label: assembled.label };
            return;
        }
        if (assembled instanceof RangeError) {
            this.loadError = { kind: "RangeError", raw: assembled.raw, label: assembled.label };
            return;
        }
        ...
        if (assembled instanceof OverlapError) {
            this.loadError = { kind: "OverlapError", codeEnd: assembled.codeEnd, stackBase: assembled.stackBase };
            return;
        }
